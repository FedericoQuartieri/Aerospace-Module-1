...
# hanno ne' workers.h ne' la condizione whole_axis. Su `unified' il backend
# Schur sta in src/tridiag/schur/ e si compila attraverso il Makefile, che e'
# quello che fa questo script. Scoprirlo qui costa un secondo, scoprirlo
# dall'errore del linker dentro il job costa una coda.
echo "=== branch ==="
printf 'HEAD:          %s\n' "$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo '(non un repo git)')"

if ! grep -q whole_axis src/tridiag/schur/momentum.c 2> /dev/null; then
    cat << 'SBAGLIATO'

ERRORE: src/tridiag/schur/momentum.c non contiene la condizione `whole_axis',
quindi questo albero non ha il multithreading del backend Schur e non c'e'
niente da diagnosticare.

Questo script vuole la disposizione di `unified' (backend in
src/tridiag/schur/, compilazione via Makefile). Su `multithread' lo stesso
codice sta ancora in src/momentum.c; `mpi', `fast' e `pipeline' non hanno
thread affatto.
...
# I 274 / 3746 / 287 ms di MULTITHREAD.md 9.2 sono stati misurati su
# `multithread' prima del porting: sono le ancore qui sotto a dire se il
# confronto regge ancora.
echo
...
# Il backend tridiagonale sta in src/tridiag/$(TRIDIAG)/ e i flag li conosce
# solo il Makefile: da qui si passano soltanto le sue variabili, come fa
# scripts/study/lib.sh. -mfma e' l'unico flag in piu' rispetto a SIMD=1, ed
# era gia' cosi'. La diagnosi riguarda i team di thread di Schur, quindi il
# backend e' quello, salvo TRIDIAG esplicito.
