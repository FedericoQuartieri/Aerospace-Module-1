...
# Il solutore condiviso piu' UN backend tridiagonale.  La politica la legge
# soltanto schur: il pipeline ha i propri cicli e non la guarda, quindi il
# confronto si fa per forza sul primo.
core_sources=()
for source in src/*.c src/tridiag/schur/*.c; do
    [[ "$(basename -- "$source")" == "main.c" ]] || core_sources+=("$source")
done

common_cflags=(-std=gnu11 -O3 -Wall -Wextra
               -Iinclude -Isrc/tridiag/schur -DTRIDIAG_SCHUR
               -fopenmp -DUSE_OMP)
