...
void solve_Dxx_tridiag_blocks(DTYPE *Eta_next_component, DTYPE *rhs, DTYPE *Gamma, 
                             VelocityField u_BC, Boolean same_direction){

    // Initialize temporary arrays 
    DTYPE *w = (DTYPE *) malloc(GRID_ELEMENTS * sizeof(DTYPE));
    DTYPE *tmp = (DTYPE *) malloc(GRID_ELEMENTS * sizeof(DTYPE));
    memset(tmp, 0, GRID_ELEMENTS * sizeof(DTYPE));
...
void solve_Dyy_tridiag_blocks(DTYPE *Zeta_next, DTYPE *rhs, DTYPE *Gamma, 
                            VelocityField u_BC, Boolean same_direction){
    ...
    if (!rhs_block || !u_block || !w_block || !tmp_block) {
        free(rhs_block); free(u_block); free(w_block); free(tmp_block);
        return;
    }
    if(same_direction){
        for (int k = 1; k < DEPTH; ++k) {
...
void solve_Dzz_tridiag_blocks(DTYPE *U_next, DTYPE *rhs, DTYPE *Gamma,
                            VelocityField u_BC, Boolean same_direction){
    // Buffer riutilizzati per ogni colonna (i,k)
