...
void compute_g(GField *g_field, ForceField *f_field, Pressure *pressure, DTYPE *K, VelocityField *Eta, VelocityField *Zeta, VelocityField *U){
    ...
    ((Eta->v_x[left_idx] - 2*Eta->v_x[idx] + (2.0 * v_boundary_x - Eta->v_x[idx])) * DX_INVERSE_SQUARE)
    g_field->g_x[idx] = f_field->f_x[idx] 
                    - compute_pressure_x_grad(pressure->p,i,j,k) 
                    - (NU / (2.0 * K[idx])) * U->v_x[idx]
                    + (NU/2.0) * (((Eta->v_x[left_idx] - 2.0*Eta->v_x[idx] + (2.0 * v_boundary_x - Eta->v_x[idx])) * DX_INVERSE_SQUARE)  
                                + ((Zeta->v_x[down_idx] - 2.0*Zeta->v_x[idx] + (2.0 * v_boundary_x - Zeta->v_x[idx])) * DY_INVERSE_SQUARE) 
                                + ((U->v_x[back_idx] - 2.0*U->v_x[idx] + (2.0 * v_boundary_x - U->v_x[idx])) * DZ_INVERSE_SQUARE) );

    g_field->g_y[idx] = f_field->f_y[idx] 
                    - compute_pressure_y_grad(pressure->p,i,j,k) 
                    - (NU / (2.0 * K[idx])) * U->v_y[idx]
                    + (NU/2.0) * (((Eta->v_y[left_idx] - 2.0*Eta->v_y[idx] + (2.0 * v_boundary_y - Eta->v_y[idx])) * DX_INVERSE_SQUARE)  
                                + ((Zeta->v_y[down_idx] - 2.0*Zeta->v_y[idx] + (2.0 * v_boundary_y - Zeta->v_y[idx])) * DY_INVERSE_SQUARE) 
                                + ((U->v_y[back_idx] - 2.0*U->v_y[idx] + (2.0 * v_boundary_y - U->v_y[idx])) * DZ_INVERSE_SQUARE) );

    g_field->g_z[idx] = f_field->f_z[idx] 
                        - compute_pressure_z_grad(pressure->p,i,j,k) 
                        - (NU / (2.0 * K[idx])) * U->v_z[idx]
                    + (NU/2.0) * (((Eta->v_z[left_idx] - 2.0*Eta->v_z[idx] + (2.0 * v_boundary_z - Eta->v_z[idx])) * DX_INVERSE_SQUARE)  
                                + ((Zeta->v_z[down_idx] - 2.0*Zeta->v_z[idx] + (2.0 * v_boundary_z - Zeta->v_z[idx])) * DY_INVERSE_SQUARE) 
                                + ((U->v_z[back_idx] - 2.0*U->v_z[idx] + (2.0 * v_boundary_z - U->v_z[idx])) * DZ_INVERSE_SQUARE) );
