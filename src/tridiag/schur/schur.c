...
void schur_solve_mpi(int axis, int lines, int n_local,
    ...
    int blocks = dims[axis]; // blocks = number of processes along the axis
    int p = coords[axis]; // p = index of the process along the axis
    ...
    // Buffer for the data of the current process
    Real *mine = xmalloc(3 * (size_t)lines * sizeof(Real));

    // Buffer for the data received from the process on the right
    ...
    // shifts the data towards the process on the left and receives the data
    // from the process on the right: this amounts to an MPI_Sendrecv, and it
    // is a single message of size 3*lines, instead of one per line. The last
    // block has no neighbour on the right, but MPI_Sendrecv handles it
    // correctly with MPI_PROC_NULL, that is the receive does nothing; in the
    // same way the first block has no neighbour on the left and the send does
    // nothing
    ...
    // allgather: that is all the processes send their own row and receive all
    // the rows of the other processes; in rows there will be all the rows of
    // the processes, in rank order
    ...
    for (int l = 0; l < lines; l++) {
        for (int q = 0; q < interfaces; q++) {
            ...
            // here is the index of the first element of row q of the reduced
            // matrix, for line l
