...
TEST(Momentum_linear_solver_test, Eta_next_computation) {
    ...
    // Solve with GMRES: mat * s = rhs, where s = Eta_next - Eta
    Eigen::GMRES<SpMat> gmres;
    gmres.setMaxIterations(maxit);
    gmres.setTolerance(tol);
    gmres.compute(mat);
    SpVec s = gmres.solve(rhs);  // s represents (Eta_next - Eta)
    ...
    double residual_norm = (mat * s - rhs).norm();
    ...
    for (int i = 0; i < WIDTH; ++i) {
        size_t idx = rowmaj_idx(i, 0, 0);  // First row: j=0, k=0
        double expected = s[i] + Eta.v_x[idx];  // Eta_next = s + Eta
    ...
    gmres_vy.compute(mat);  // Same matrix as v_x (WIDTH x WIDTH, DX_INVERSE_SQUARE)
    SpVec s_vy = gmres_vy.solve(rhs_vy);

    double residual_norm_vy = (mat * s_vy - rhs_vy).norm();
    std::cout << "residual norm (v_y) ||As-rhs||: " << residual_norm_vy << std::endl;

    for (int i = 0; i < n; ++i) {
        size_t idx = rowmaj_idx(i, 0, 0);
        double expected = s_vy[i] + Eta.v_y[idx];
    ...
    gmres_vz.compute(mat);  // Same matrix as v_x (WIDTH x WIDTH, DX_INVERSE_SQUARE)
    SpVec s_vz = gmres_vz.solve(rhs_vz);
    double residual_norm_vz = (mat * s_vz - rhs_vz).norm();
    std::cout << "residual norm (v_z) ||As-rhs||: " << residual_norm_vz << std::endl;

    for (int i = 0; i < n; ++i) {
        size_t idx = rowmaj_idx(i, 0, 0);
        double expected = s_vz[i] + Eta.v_z[idx];
...
TEST(Momentum_linear_solver_test, Zeta_next_computation) {
    ...
    SpVec rhs(n);
    for (int i = 0; i < n; ++i) {
        size_t idx = rowmaj_idx(0, i, 0);  // First row: j=0, k=0
    ...
    for (int i = 0; i < HEIGHT; ++i) {
        size_t idx = rowmaj_idx(0, i, 0);  // First row: j=0, k=0
        double expected = s[i] + Zeta.v_x[idx];  // Eta_next = s + Eta
    ...
    for (int i = 0; i < n; ++i) {
        ...
        double expected = s_vy[i] + Zeta.v_y[idx];
    ...
    for (int i = 0; i < n; ++i) {
        size_t idx = rowmaj_idx(0, i, 0);
        double expected = s_vz[i] + Zeta.v_z[idx];
...
TEST(Momentum_linear_solver_test, U_next_computation) {
    ...
    SpVec rhs(n);
    for (int i = 0; i < n; ++i) {
        size_t idx = rowmaj_idx(0, 0, i);  // First row: j=0, k=0
    ...
    for (int i = 0; i < DEPTH; ++i) {
        size_t idx = rowmaj_idx(0, 0, i);  // First row: j=0, k=0
        double expected = s[i] + U.v_x[idx];  // Eta_next = s + Eta
    ...
    for (int i = 0; i < n; ++i) {
        ...
        double expected = s_vy[i] + U.v_y[idx];
    ...
    for (int i = 0; i < n; ++i) {
        size_t idx = rowmaj_idx(0, 0, i);
        double expected = s_vz[i] + U.v_z[idx];
