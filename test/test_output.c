...
int main(void)
{
    ...
    file = fopen("test-output/solution_000001.vti", "rb");
    if (file == NULL) goto cleanup;
    bytes = fread(header, 1, sizeof(header) - 1, file);
    fclose(file);
    header[bytes] = '\0';
    if (strstr(header, "Name=\"TimeValue\"") == NULL ||
        strstr(header, "Name=\"PressureTime\"") == NULL ||
...
cleanup:
    ...
    remove("test-output/solution_000001.vti");
    rmdir("test-output");
    if (result == 0) {
